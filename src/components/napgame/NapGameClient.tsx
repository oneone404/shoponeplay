...
export default function NapGameClient({ initialHotConfig, logoUrl, topupProducts = [] }: NapGameClientProps) {
      ...
      try {
        if (isBulkMode) {
          ...
          if (topupProduct && character) {
            ...
            console.log("[DEBUG_NAPGAME] Sending manual product:", manualProductData);
  ...
  useEffect(() => {
    // Khong poll neu khong co ID hoac la don nap tay (Admin duyet)
    if (!topupOrderId || topupStatus === "WAITING_ADMIN_PAY") return
  ...
  }, [topupOrderId, topupStatus, addMessage])
  ...
  const displayProducts = useMemo(() => {
    let list = [...(groupedProducts[activeTab] || [])]
    ...
    if (priceSort !== 'none') {
      list.sort((a, b) => {
        const priceA = a.prices.VND.price
        const priceB = b.prices.VND.price
        return priceSort === 'asc' ? priceA - priceB : priceB - priceA
      })
    }

    return list
  }, [groupedProducts, activeTab, localSearch, priceSort])
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <UserPageHeader 
        title="Nạp Gói Game" 
        subtitle="Hệ Thống Nạp Gói Game Chính Thức"
      />
        ...
        {(hasSearched || isPending) && (
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700 min-h-[400px]">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-1 bg-white dark:bg-card p-1 border border-border rounded-xl shadow-sm w-fit max-w-full">
                {tabs.map(tab => (
                  <button
                    key={tab}
                    onClick={() => {
                      setActiveTab(tab)
                      setLocalSearch("") // Reset search when changing tab
                    }}
                    className={cn(
                      "px-4 py-2 rounded-lg font-bold uppercase tracking-widest text-[9px] sm:text-[10px] transition-all",
                      activeTab === tab
              ...
              <div className="flex flex-col gap-4 items-center md:items-start animate-in fade-in slide-in-from-top-4 duration-500">
                <div className="relative w-full md:w-80 group">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-card border border-border rounded-xl text-xs font-bold focus:border-primary outline-none transition-all shadow-sm"
                  />
                </div>

                <div className="flex items-center gap-2 w-full md:w-80">
                  <div className="flex items-center gap-1 bg-white dark:bg-card p-1 border border-border rounded-xl shadow-sm w-full">
                    <button
                      onClick={() => setPriceSort('none')}
                      className={cn(
                        "flex-1 px-4 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all",
                        priceSort === 'none' ? "bg-primary text-white" : "text-muted-foreground hover:bg-secondary"
                      )}
                    >
                      Mặc định
                    </button>
                    <button
                      ...
                      className={cn(
                        "flex-1 px-4 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all",
                        priceSort === 'asc' ? "bg-primary text-white" : "text-muted-foreground hover:bg-secondary"
                      )}
                    >
                      Giá ↑
                    </button>
                    <button
                      ...
                      className={cn(
                        "flex-1 px-4 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all",
                        priceSort === 'desc' ? "bg-primary text-white" : "text-muted-foreground hover:bg-secondary"
                      )}
                    >
                      Giá ↓
                    </button>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
              {isPending ? (
                ...
                <div className="col-span-full py-20 text-center space-y-4 opacity-50">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest">Không tìm thấy gói nạp phù hợp</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="relative w-full max-w-2xl bg-card border-2 border-border rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500 flex flex-col max-h-[90vh]">
              ...
              <div className="p-6 sm:p-8 border-t border-border bg-secondary/5 flex flex-col sm:flex-row items-center justify-between gap-6">
                ...
                <button
                  ...
                  disabled={isPending}
                  className="w-full sm:w-auto px-8 h-12 bg-background border border-border text-foreground rounded-2xl font-bold uppercase tracking-widest hover:bg-secondary active:scale-95 shadow-sm transition-all flex items-center justify-center gap-1.5 text-[11px] group/btn"
                >
            ...
            <div className="relative w-full max-w-md bg-card border-2 border-border rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
              <div className="p-8 space-y-6">
                ...
                <div className="text-center space-y-1">
                  <h3 className="text-lg font-bold uppercase tracking-tight text-foreground">Xác nhận nạp gói</h3>
                  <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-[0.2em] opacity-70">
                    Kiểm tra kỹ thông tin trước khi nạp
                  </p>
                </div>

                {/* Product Info Box */}
                <div className="bg-white dark:bg-card border border-border rounded-2xl p-4 flex items-center space-x-4 shadow-sm">
                  <div className="w-14 h-14 rounded-xl overflow-hidden border border-border shrink-0">
                    <img src={confirmingProduct.image} alt="" className="w-full h-full object-cover" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-[11px] font-bold text-foreground truncate uppercase mb-0.5">{confirmingProduct.productName}</p>
                    <div className="flex items-baseline space-x-1">
                      ...
                      <span className="text-[9px] font-bold text-accent/70 uppercase">VND</span>
                    </div>
                  </div>
                </div>

                {/* Selection Mode Toggle (Grouped Style) */}
                <div className="flex items-center justify-center">
                  <div className="inline-flex p-1 bg-white dark:bg-card border border-border rounded-xl shadow-sm">
                    <button
                      onClick={() => setIsBulkMode(false)}
                      className={cn(
                        "px-4 py-2 rounded-lg text-[9px] font-bold uppercase tracking-widest transition-all",
                        !isBulkMode ? "bg-primary text-white shadow-md shadow-primary/20" : "text-muted-foreground hover:bg-secondary"
                      )}
                    >
                      Nạp ID hiện tại
                    </button>
                  ...
                  <div className="space-y-3">
                    <div className="relative group">
                      <textarea
                        ...
                        className="w-full h-32 p-4 bg-white dark:bg-card border border-border rounded-2xl focus:border-primary font-bold text-xs transition-all outline-none resize-none custom-scrollbar shadow-sm"
                        value={bulkIdsInput}
                        onChange={(e) => setBulkIdsInput(e.target.value)}
                      />
                      <div className="absolute right-3 bottom-3 p-1.5 bg-secondary/50 rounded-lg">
                        <Zap className="w-3 h-3 text-primary" />
                      </div>
                    </div>
                    <div className="flex items-center justify-between px-1">
                      <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                        Đã nhập: <span className="text-primary">{bulkIds.length} ID</span>
                      </p>
                      <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">
                        Tổng tiền tự động x{bulkIds.length}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col items-center justify-center py-4 bg-secondary/20 rounded-2xl border border-dashed border-border/50 space-y-1.5 px-6">
                    <div className="flex items-center space-x-2">
                      <User className="w-3.5 h-3.5 text-primary" />
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Tài khoản nạp: </span>
                    </div>
                    <div className="text-center">
                      <span className="block text-sm font-bold text-foreground tracking-tight leading-tight">{character?.name}</span>
                      <span className="block text-[10px] font-bold text-muted-foreground/70 uppercase tracking-widest mt-0.5">ID: {character?.id}</span>
                    </div>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button
                    ...
                    className="h-12 bg-background border border-border rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:bg-secondary active:scale-95 transition-all disabled:opacity-50 shadow-sm"
                  >
                    Hủy bỏ
                  </button>
                  <button
                    ...
                    className="h-12 bg-primary text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest hover:opacity-90 active:scale-95 transition-all flex items-center justify-center disabled:opacity-50"
                  >
                    {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : (
                      <div className="flex items-center space-x-2">
                        <span>Xác nhận</span>
      ...
      <div className="p-3 sm:p-4 flex flex-col flex-1 space-y-3 text-center">
        ...
        <div className="pt-3 border-t border-border mt-auto space-y-3">
          ...
          <button
            ...
            className="w-full py-2.5 bg-background text-foreground border border-border rounded-xl text-[10px] font-bold uppercase tracking-widest text-center flex items-center justify-center gap-1.5 hover:bg-secondary active:scale-95 transition-all shadow-sm group/btn"
          >
            <span>Nạp Ngay</span>
