...
export default function SellerProductForm({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: "thumbnail" | "detail") => {
    ...
    try {
      ...
      // Use the general upload API or a seller-specific one
      ...
      addMessage({ type: "success", text: "Tải ảnh thành công" })
    } catch (err) {
      setError(err instanceof Error ? err.message : "Có lỗi khi upload")
      addMessage({ type: "error", text: "Lỗi tải ảnh" })
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We will create /api/seller/products
      ...
      addMessage({ 
        type: "success", 
        text: productId ? "Cập nhật sản phẩm thành công" : "Đăng sản phẩm thành công" 
      })
