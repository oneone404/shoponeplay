...
import QuickViewAccountsModal from "../../admin/products/QuickViewAccountsModal"
...
export default function SellerProductsClient({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/seller/products/${deletingProduct.id}`, { method: 'DELETE' })
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-secondary/50 text-[10px] uppercase tracking-widest text-muted-foreground border-b border-border">
              <tr>
                <th className="px-6 py-4 font-bold text-left">Sản phẩm</th>
                <th className="px-6 py-4 font-bold text-center">Loại</th>
                {!hideAccountColumn && <th className="px-6 py-4 font-bold text-left">Tài khoản</th>}
                <th className="px-6 py-4 font-bold">Giá bán</th>
                <th className="px-6 py-4 font-bold text-center">Trạng thái</th>
                {!hideStockColumn && <th className="px-6 py-4 font-bold text-center">Kho hàng</th>}
                  ...
                  <tr key={product.id} className={cn("hover:bg-secondary/20 transition-colors group", product.isHidden && "opacity-60 bg-secondary/10")}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-muted-foreground font-medium uppercase truncate max-w-[150px]">ID: {product.id.slice(-8)}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className={cn(
                        "inline-block px-2 py-1 rounded-md text-[9px] font-black uppercase tracking-tighter border",
                        product.type === "PLAY" 
                          ? "bg-blue-500/10 text-blue-500 border-blue-500/20" 
                          : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                      )}>
                        {product.type === "PLAY" ? "PLAY" : "RANDOM"}
                      </span>
                    </td>
                    {!hideAccountColumn && (
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex flex-col items-center gap-2">
                        {product.type === "PLAY" && product.sold ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-secondary text-muted-foreground/50 rounded-lg border border-border text-[10px] font-black uppercase tracking-widest">
                            <CheckCircle className="w-3 h-3" /> Đã bán
                          </span>
                        ) : (
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Link href={SELLER_ROUTES.EDIT_PRODUCT(product.id).path} className="p-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg transition-all"><Edit className="w-4 h-4" /></Link>
      ...
      <ConfirmModal
        ...
        title="Xác nhận xóa"
        message="Mọi dữ liệu liên quan sẽ bị xóa vĩnh viễn."
