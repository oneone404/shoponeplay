...
export default function SellerLayoutShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background relative overflow-hidden">
      ...
      <div className={cn(
        ...
        <SellerSidebar logoUrl={logoUrl} onCloseMobile={() => setIsSidebarOpen(false)} />
      </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="sticky top-0 z-30 flex items-center justify-between p-4 bg-card border-b border-border md:hidden shrink-0">
          <div className="flex items-center space-x-2">
            ...
            />
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] bg-foreground/90 text-background px-2 py-1 rounded-md">Seller</span>
          </div>
          <button 
